...
export function SocialIdeaModal({ idea, open, onOpenChange, onIdeaUpdated, onIdeaDeleted, users, calendarType = 'social' }: SocialIdeaModalProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) { onIdeaDeleted(); onOpenChange(false); }
      ...
      <DialogContent className="sm:max-w-5xl max-h-[90vh] overflow-hidden flex flex-col rounded-2xl">
          ...
          <div className="flex-1 min-w-0">
            ...
            <DialogDescription className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {(() => {
                ...
                const Icon = cfg.icon;
                return (
                  <Badge variant="outline" className={cn('text-[10px] gap-1 border-0', cfg.bgColorClass, cfg.colorClass)}>
                    <Icon className="h-2.5 w-2.5" /> {cfg.label}
                  </Badge>
                );
              })()}
              <Badge variant="outline" className="text-[10px] border-0 bg-muted/40">{idea.assignees?.length || 0} asignados</Badge>
            </DialogDescription>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            <Button variant="ghost" size="sm" className="rounded-lg" onClick={copyIdeaLink} title="Copiar link de la idea">
              <Copy className="h-3.5 w-3.5" /> {copiedLink ? 'Copiado' : null}
            </Button>
            {!editing ? (
              <Button variant="ghost" size="sm" className="rounded-lg" onClick={() => setEditing(true)}>
        ...
        <div className="shrink-0">
          ...
          <div className="flex gap-1.5 flex-wrap">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? cn(s.colorClass, 'bg-current/10 font-semibold') : 'bg-muted/35 text-muted-foreground/70 hover:bg-muted/55',
            ...
            {changingStatus && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground self-center" />}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              {editing ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={ejeContenido} onChange={(e) => setEjeContenido(e.target.value)} placeholder="Eje de contenido..." className="rounded-lg bg-muted/35 border-0" />
                  </div>
                        ...
                        return (
                          <button key={pt.value} type="button" onClick={() => setPostType(pt.value)}
                            className={cn('flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs transition-colors',
                              postType === pt.value ? cn(cfg.bgColorClass, cfg.colorClass, 'font-semibold') : 'bg-muted/35 text-muted-foreground hover:bg-muted/55')}>
              ...
              {isSocial && (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Input placeholder="Pegá un link de Drive..." value={newAttachUrl} onChange={(e) => setNewAttachUrl(e.target.value)}
                  ...
                  {attachments.length > 0 && (
                    <div className="space-y-1.5">
                      {attachments.map(att => (
                        <div key={att.id} className="flex items-center justify-between gap-2 rounded-xl bg-muted/30 p-2 transition-colors hover:bg-muted/50">
            ...
            {isSocial && (
              <div className="md:col-span-2 flex flex-col min-h-0 rounded-2xl bg-muted/25 p-3.5">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium flex items-center gap-1 mb-2 shrink-0">Comentarios ({comments.length})</p>
                <div className="flex-1 overflow-y-auto min-h-0">
                  <SocialComment ideaId={idea.id} comments={comments} onAddComment={addComment} onDeleteComment={deleteComment} onUpdateComment={updateComment} currentUserId={user?.id} />
                </div>
              </div>
            )}
          </div>
