...
- **CTA buttons**: `Button` variant+size `cta` (`h-10`, `rounded-xl`); `.btn-cta` in globals.css (gradient 135°, `::before` shine-sweep animation, hover lift + glow, active press). Applied to primary actions: Nuevo documento, Nueva Tarea, Nueva Idea, Crear usuario, Guardar perfil, login submit buttons. `.no-scrollbar` utility added (scrollbar-width: none).
- **Sidebar rail scroll**: nav area is a plain `overflow-y-auto no-scrollbar` div (no ScrollArea). Floating gradient chevron button at bottom-center scrolls by a page (`ChevronDown`/`ChevronUp` depending on position) + bottom gradient fade when scrollable; active item auto-scrolls into view via `data-active` + `scrollIntoView`.
